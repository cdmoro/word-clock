...
const commonWords: CommonWords = {
  ...
  FIVE_MIN: (_hours, minutes) => (minutes >= 55 ? [106, 107, 108, 109] : [40, 41, 42, 43]), // CINC (cinco minutos)
...
const localeWords = {
  ...
  SON: [1, 2, 3], // SÓN
  LES: [14, 15, 16],
  MENYS: [99, 100, 101, 102, 103], // MENYS
  I: [38], // I
...
function getLocaleWordKeys(hours: number, minutes: number) {
  ...
  else if ((minutes >= 15 && minutes < 20) || (minutes >= 30 && minutes < 35) || (minutes >= 45 && minutes < 50))
    wordKeys.push('SON');
  else wordKeys.push('SON', 'LES');

  if (minutes >= 5 && minutes < 10) wordKeys.push('I', 'FIVE');
  else if (minutes >= 10 && minutes < 15) wordKeys.push('I', 'TEN');
  else if (minutes >= 15 && minutes < 20) wordKeys.push('ÉS', 'UN', 'QUART', 'DE');
  else if (minutes >= 20 && minutes < 25) wordKeys.push('ÉS', 'UN', 'QUART', 'DE', 'I', 'FIVE_MIN');
  else if (minutes >= 30 && minutes < 35) wordKeys.push('DOS', 'QUARTS', 'I', 'FIVE_MIN', 'DE');
  else if (minutes >= 35 && minutes < 40) wordKeys.push('DOS', 'QUARTS', 'DE');
  else if (minutes >= 40 && minutes < 45) wordKeys.push('DOS', 'QUARTS', 'DE', 'I', 'TEN_MIN', 'DE');
  else if (minutes >= 45 && minutes < 50) wordKeys.push('TRES', 'QUARTS', 'DE');
  else if (minutes >= 50 && minutes < 55) wordKeys.push('MENYS', 'FIVE_MIN');
  else wordKeys.push('MENYS', 'FIVE_MIN');
